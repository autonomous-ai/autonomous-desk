...
SCAN_THROTTLE_PATH = os.path.join(device.CONFIG_DIR, "autonomous-lcd-scan.last")
SCAN_THROTTLE_SECONDS = 60
# Warning rate-limit: don't nag the user on every event while the device is down.
WARN_PATH = os.path.join(device.CONFIG_DIR, "autonomous-lcd-warn.last")
WARN_SECONDS = 600
...
def warn_user(message):
    """Surface a non-blocking warning to the Codex user via systemMessage.
